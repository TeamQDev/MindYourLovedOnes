...
public class DosageQuery {
    ...
    public static String createDosageTable() {
        String createTableQuery = "create table  If Not Exists " + TABLE_NAME + "(" + COL_ID + " INTEGER PRIMARY KEY,"+ COL_USERID + " INTEGER,"+ COL_MEDICINE + " VARCHAR(20)," + COL_DOSE+ " VARCHAR(10),"+ COL_PREID + " integer,"
                + " FOREIGN KEY ("+COL_PREID+") REFERENCES "+PrescriptionQuery.TABLE_NAME+"("+PrescriptionQuery.COL_ID+"));";
    ...
    public static ArrayList<Dosage> fetchAllDosageRecord(int userid, int prid) {
        ArrayList<Dosage> noteList=new ArrayList<>();
        SQLiteDatabase db=dbHelper.getReadableDatabase();
        Cursor c=db.rawQuery("select * from "+TABLE_NAME + " where " + COL_USERID + "='" + userid+"' and "+COL_PREID+"='"+prid+"';",null);
    ...
    public static Boolean insertDosageData(int userid, ArrayList<Dosage> dosageList) {
        boolean flag=false;
        SQLiteDatabase db = dbHelper.getWritableDatabase();
